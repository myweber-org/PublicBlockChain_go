...
func (rl *RotatingLogger) compressPrevious() {
	if rl.fileCount <= 1 {
		return
	}
	prevNum := rl.fileCount - 1
	source := fmt.Sprintf("%s_%d.log", rl.basePath, prevNum)
	dest := fmt.Sprintf("%s_%d.log.gz", rl.basePath, prevNum)
...
func main() {
	...
	files, _ := filepath.Glob("app_log_*")
