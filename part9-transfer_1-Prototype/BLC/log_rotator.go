...
func (lr *LogRotator) rotate() error {
    ...
    err = compressFile(backupName, compressedName)
    if err != nil {
        return err
    }
...
func main() {
    ...
    fmt.Printf("Rotation complete. Backups: %d\n", len(rotator.backupList))
}
package main
...
func (lr *LogRotator) compressOldLog() {
	dir := filepath.Dir(lr.basePath)
	pattern := filepath.Base(lr.basePath) + "_*.log"
...
func main() {
	rotator, err := NewLogRotator("/var/log/myapp/app", 10)
	if err != nil {
		fmt.Printf("Failed to create log rotator: %v\n", err)
		return
	}
...
func (lr *LogRotator) cleanupOldBackups() {
    ...
    for _, match := range matches {
        timestampStr := strings.TrimSuffix(strings.TrimPrefix(match, lr.basePath+"."), ".gz")
        timestamp, err := strconv.ParseInt(timestampStr, 10, 64)
        if err == nil {
            backupFiles = append(backupFiles, backupFile{path: match, timestamp: timestamp})
        }
    }
