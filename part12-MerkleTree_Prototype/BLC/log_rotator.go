...
func (rl *RotatingLogger) rotate() error {
    ...
    rl.currentNum++
    if rl.currentNum > maxBackupFiles {
        rl.currentNum = 1
    }

    oldPath := rl.getBackupPath(rl.currentNum)
    if err := compressFile(rl.basePath, oldPath); err != nil {
        return err
    }
...
func (rl *RotatingLogger) getBackupPath(num int) string {
    ...
    timestamp := time.Now().Format("20060102_150405")
    return fmt.Sprintf("%s_%s_%d%s.gz", base, timestamp, num, ext)
}
...
func cleanupOldBackups() {
    files, err := filepath.Glob("app_*.log.gz")
    if err != nil {
        return
    }
