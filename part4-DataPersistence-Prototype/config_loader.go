...
func getIntEnv(key string, defaultValue int) (int, error) {
    if value := os.Getenv(key); value != "" {
        ...
        if err != nil {
            return 0, fmt.Errorf("cannot convert %s to integer: %w", value, err)
        }
        return intValue, nil
    }
...
func getBoolEnv(key string, defaultValue bool) (bool, error) {
    if value := os.Getenv(key); value != "" {
        ...
        if err != nil {
            return false, fmt.Errorf("cannot convert %s to boolean: %w", value, err)
        }
...
func validateConfig(cfg *Config) error {
    if cfg.ServerPort < 1 || cfg.ServerPort > 65535 {
        return fmt.Errorf("server port %d is out of valid range (1-65535)", cfg.ServerPort)
    }
    
    if cfg.DBPort < 1 || cfg.DBPort > 65535 {
        return fmt.Errorf("database port %d is out of valid range (1-65535)", cfg.DBPort)
    }
...
func LoadConfig() (*Config, error) {
    ...
    cfg.DatabaseURL = getEnvString("DATABASE_URL", "postgres://localhost:5432/app")
    cfg.LogLevel = getEnvString("LOG_LEVEL", "info")
    cfg.CacheEnabled = getEnvBool("CACHE_ENABLED", true)
    cfg.MaxConnections, err = getEnvInt("MAX_CONNECTIONS", 100)
    if err != nil {
