...
func (rl *RotatingLogger) shouldRotate(incomingSize int) bool {
    if rl.fileSize+int64(incomingSize) > maxFileSize {
        return true
    }
    
    if time.Since(rl.backupTime).Hours() >= 24 {
        return true
    }
...
func main() {
    ...
    for i := 0; i < 1000; i++ {
        log.Printf("Log entry %d: %s", i, generateTestData())
        time.Sleep(100 * time.Millisecond)
    }
}
